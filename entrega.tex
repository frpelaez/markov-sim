...
\lstset{ %
  ...
  morekeywords={None, self, as, ndarray},               % if you want to add more keywords to the set
...
\textbf{NOTA}: Las viñetas de código mostradas en este informe no muestran todo el código utilizado para la resolución de los ejercicios, únicamente la parte más relevante, evitando sobrecargar este documento con ciertos detalles de la implementación. En cualquier caso, el código completo puede encontrarse en el siguiente \href{https://github.com/frpelaez/markov-sim}{enlace}.

\begin{lstlisting}[title=En src/simulation.py]
    import numpy as np
...
\begin{lstlisting}[title={En src/simulation.py, después de \_\_init\_\_()}]
        def run(self) -> None:
            ...
...
\begin{lstlisting}[title={En src/simulation.py, después de run()}]
...
\begin{lstlisting}[title={En src/simulation.py, después de \_compute\_paths()}]
...
\begin{lstlisting}[title={En src/simulation.py, después de \_estimate\_final\_distribution()}]
